...
#define S_FUNCTION_NAME  Inv_kinematic
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    *teta1 = phi+acos((cos(beta)-cos(gama)*cos(alpha))/(sin(gama)*sin(alpha)));
    *teta2 = phi-acos((cos(beta)-cos(gama)*cos(alpha))/(sin(gama)*sin(alpha)));
